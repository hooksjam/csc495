...
class QueryPage extends React.Component {
    ...
    async sendQuery(e) {
        ...
        await this.props.sendESQuery(query);
    }
    ...
    getTestQueries() {
        if(this.props.query && this.props.query.test) {
            return this.props.query.test.map(x => {
                return (
                    <Tooltip title={x.description}>
                        <Button 
                            ...
                            onClick={() => {
                                this.setState({query:JSON.stringify(x.queries, undefined, 4)
                            })}}>{x.excel}</Button>
                    </Tooltip>
                )
            })
            ...
            <Grid className="queryWrapper" container spacing={3}>
                ...
                <Grid className="content">
                    ...
                    <Grid item xs={12} style={{marginTop:10}}>
                        <Button variant="contained" color="primary" type="button" style={{width: '100%', padding:5}} onClick={this.sendQuery}>Run Query</Button>
                    </Grid>

                    <Grid className="query" item xs={12} style={{marginTop:10}}>
                    ...
                    <Grid item xs={12} style={{marginTop:10}}>
                        ...
                        <Card style={{marginTop: 10}}>
                            <CardContent>
                                <Typography variant="h4" onClick={()=>{onClick("summary")}}>Results</Typography>
                                {this.props.query.result && this.props.query.result.map(x => {
                                    return <a href={`/api/query/result/${x}`} target="_blank"><h5>{x}</h5></a>
                                })}
                                <pre>
                                {JSON.stringify(this.props.query.raw, undefined, 2)}
                                </pre>
                            </CardContent>
                        </Card> 
                    </Grid>  
                    ...
                    {/*<Grid item xs={12} style={{marginTop:10}}>
                        <Result 
                            answer={this.props.query && this.props.query.result && this.props.query.result.answer}
                            query={this.props.query && this.props.query.result && this.props.query.result.query}
                        />
                    </Grid>*/}
...
const actionCreators = {
    ...
    //getQuery: FormActions.getFormQuery,
    sendESQuery: QueryActions.sendESQuery,
    //logout: UserActions.logout
}

const connectedQueryPage = connect(mapState, actionCreators)(QueryPage);
export { connectedQueryPage as QueryPage };
